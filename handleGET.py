...
def generate_directory_listing_html(path):
    ...
    document.getElementById("uploadForm").addEventListener("submit", function (event) {
        ...
        var dirValue = document.getElementById("dir").value;
        var actionUrl = "./upload?path=" + encodeURI(dirValue);
...
<hr>
<label for="dir">Upload to:</label><br/>
...
def response_206(client_socket,abs_path,range_header, cookie):
    ...
    client_socket.sendall(response_headers.encode() + multipart_content)
    return
...
def HandleGET(client_socket,request_data, cookie = None):
    # print("request_data", request_data)
    request_line = request_data.splitlines()[0].decode()

    # check "Range" exist or not
